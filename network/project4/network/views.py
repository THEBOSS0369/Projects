...
def remove_like(request, post_id):
    ...
    like.delete()
    return JsonResponse({"message": "Like Deleted Successful"})
...
def add_like(request, post_id):
    ...
    newLike.save()
    return JsonResponse({"message": "Like Added Successful"})
...
def index(request):
    ...
    #paginator for the next and previous page
    ...
    try:
        for like in allLikes:
            if like.used.id == request.user.id:
...
def profile(request, user_id):
    ...
    try:
        checkFollow = Follow.objects.filter(user=User.objects.get(pk=request.id)) 
    ...
    #pageinator for the next and previous page
    paginator = Paginator(allPosts, 10)
    page_number = request.GET.get('page')
    posts_of_the_page= paginator.get_page(page_number)
